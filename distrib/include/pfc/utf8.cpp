...
static int compare_wchar(unsigned c1,unsigned c2)
{
	c1 = char_lower(c1);
	c2 = char_lower(c2);
	if (c1<c2) return -1;
...
int stricmp_utf8(const char * p1,const char * p2)
{
	for(;;)
	{
...
int strlen_utf8(const char * p,int num)
{
	unsigned int w,d;
	int ret = 0;
	for(;num;)
...
int stricmp_utf8_partial(const char * p1,const char * p2,int num)
{
	for(;num;)
	{
		unsigned int w1,w2,d1,d2;
...
void string_base::add_string_lower(const char * src,unsigned len)
{
	while(*src && len)
	{
		unsigned int c,d;
		...
		if (d==0 || d>len) break;
		add_char(char_lower(c));
		src+=d;
		len-=d;
	}
}

void string_base::add_string_upper(const char * src,unsigned len)
{
	while(*src && len)
	{
		unsigned int c,d;
...
int stricmp_utf8_max(const char * p1,const char * p2,unsigned p1_bytes)
{
	for(;;)
	{
		if (p1_bytes==0 || (*p1==0 && *p2==0)) return 0;
		else if (*p1>0 && *p2>0)//signed char
		{
			unsigned c1 = q_tolower(*p1), c2 = q_tolower(*p2);
			if (c1<c2) return -1;
		...
		else
		{
			...
			d2 = utf8_decode_char(p2,&w2);
			if (w1==0 && w2==0 || d1>p1_bytes) return 0;
			int rv = compare_wchar(w1,w2);
			if (rv) return rv;
			p1 += d1;
			p2 += d2;
